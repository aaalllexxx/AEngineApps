...
- **Выделение безопасности**: Инструменты `enable_cors` и `enable_csp` удалены из ядра `App` (`AEngineApps`). Теперь они являются частью автономного модуля безопасности `sec` (`sec.sys_protect`), чтобы не перегружать базовый фреймворк функциями, специфичными для защиты.
